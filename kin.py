...
class Kinematics(object):
    ...
    def inverse_all(self, x, q_guess=None, q_min=6*[-2.*np.pi], q_max=6*[2.*np.pi], test = False):
        x_mat = np.array(x)
        if x_mat.shape != (4,4):
            print 'q should be array-like of length 6'
            return None
    ...
    def inverse(self, x, q_guess=None, q_min=6*[-2.*np.pi], q_max=6*[2.*np.pi], weights=6*[1.], test = False):
        x_mat = np.array(x)
        if x_mat.shape != (4,4):
            print 'q should be array-like of length 6'
            return None
	...
	#best_sol_project = closest_sols[5]
        if False:
	    ...
	    #print 'best_sol_project', best_sol_project
	if test == True:
		return closest_sols
        return best_sol
